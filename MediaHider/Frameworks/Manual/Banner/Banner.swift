...
open class Banner: UIView {
    open class func topWindow() -> UIWindow? {
...
// MARK: Customised Banner

extension Banner
{
    public enum enumAlertColor {
        case error
        case info
        case success
    }
    
    static var customBanner : Banner!
    
    public static func customBannerShow(title : String ,subtitle : String , colorCase : enumAlertColor , didTapBlock: (() -> ())? = nil)
    {
        var color : UIColor!
        
        if customBanner != nil
        {
            customBanner.removeFromSuperview()
        }
        ...
        else if colorCase == .info
        {
            color = UIColor.PR_hex(netHex: "#FF9233")
        }
        else
        {
            color = UIColor.PR_hex(netHex: "#88B306")
        }
        
        customBanner = Banner(title: title, subtitle: subtitle, image: nil, backgroundColor: color, didTapBlock: didTapBlock)
        
        customBanner.dismissesOnTap = true
        customBanner.show(duration: 3.0)
    }
}
