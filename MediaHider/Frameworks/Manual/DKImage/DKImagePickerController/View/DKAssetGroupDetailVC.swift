...
open class DKAssetGroupDetailVC: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, DKGroupDataManagerObserver {
    ...
    override open func viewDidLoad() {
        ...
        let barButton = UIBarButtonItem(title: "SelectAll", style: UIBarButtonItemStyle.plain, target: self, action: #selector(selectAllPhotos))
        self.navigationItem.leftBarButtonItem = barButton
        
        self.leftBarButtonItem = barButton
        
        self.isAllImageSelected = false
    }
    ...
    var isAllImageSelected = false
    
    @objc open func selectAllPhotos() {
        
        if isAllImageSelected
        {
            for i in 0..<collectionView(self.collectionView, numberOfItemsInSection: 0)
            {
                if let asset = self.fetchAsset(for: i)
                {
                    self.imagePickerController.deselectAsset(asset)
                }
            }
            
            leftBarButtonItem?.title = "SelectAll"
        ...
        else
        {
            ...
            leftBarButtonItem?.title = "SelectNone"
            
            isAllImageSelected = true
        }
